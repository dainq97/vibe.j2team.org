...
<template>
  <!-- Theme toggle -->
  <button
    class="btn fixed top-4 right-4 z-50"
    @click="toggleTheme"
  >
  ...
  <div class="wrap max-w-5xl mx-auto flex flex-wrap gap-5 p-6">
    ...
    <div class="panel flex-1 min-w-[340px]">
      ...
      <footer class="text-xs text-gray-400 mt-3">
        Supports data URLs (<code>data:image/...;base64,...</code>) or raw base64. Small images recommended.
      </footer>
    </div>
  ...
  <a href="https://linktr.ee/bowtuckle" target="_blank" rel="noopener noreferrer" class="btn secondary fixed bottom-4 right-4 z-50 blink">
    Our System
  </a>
</template>
...
@keyframes pulse-border {
  0%, 100% { box-shadow: 0 0 0 0 rgba(255, 158, 203, 0.7); }
...
.btn:hover { background: #ff6cab; transform: translateY(-1px); }
...
.btn.secondary:hover { background: #ff9ecb; color: white; }
...
.upload-btn {
  ...
  font-weight: 600;
  font-size: 14px;
  box-shadow: 0 4px 12px rgba(255,140,193,.4);
  transition: transform .2s, box-shadow .2s;
  user-select: none;
}
.upload-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 14px rgba(255,140,193,.55); }
...
body.dark-theme { background: #2b1f3b; color: #eee; }

body.dark-theme .panel {
  background: #3a2a4d;
  border-color: #5b3a7a;
  box-shadow: 0 3px 15px rgba(200,150,255,.25);
}
body.dark-theme h1 { color: #a463d9; }
...
body.dark-theme .btn        { background: #d29bff; }
body.dark-theme .btn:hover  { background: #a463d9; }
body.dark-theme .btn.secondary {
  background: #3a2a4d;
  color: #d29bff;
  border-color: #d29bff;
}
body.dark-theme .btn.secondary:hover { background: #d29bff; color: white; }
</style>
